...
public class UserVO {
	...
	/*
	 한 명의 회원은 글을 여러개 작성할 수 있음 
	 마이 페이지에서는 특정 회원이 작성한 글 목록을 나타내야 함
	 회원 정보와 글 목록은 서로 다른 테이블로 이루어져 있고 마이페이지에서는 
	 해당 정보를 한번의 DB 연동으로 가져올 수 있도록 하기 위해 JOIN문법으로 테이블을 합친 뒤 
	 원하는 컬럼을 선택해서 가져올 예정 
	 */
	
	// 1이 UserVO이기 때문에 UserVO안에 N의 값을 뜻하는 FreeBoardVO 객체의 모듈을 
	// 저장할 수 있는 List 선언 
